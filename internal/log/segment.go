...
func newSegment(dir string, baseOffset uint64, c Config) (*segment, error) {
	...
	/*
		    The below code checks if there was an error while reading the index entry. If an error occurred, it means there are no
			entries in the index, and thus, no records have been appended yet. If there is an error (no entries), the s.nextOffset is
			set to the baseOffset. This means that the next record to be appended will start at the base offset of the segment.

			If there is no error (entries exist), it means there
